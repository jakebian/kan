...
ScrollSpy.prototype.getElemsViewState = function () {
  ...
  for (var i = 0, max = this.contents.length; i < max; i++) {
    var currentContent = this.contents[i],
      isInView = this.isInView(currentContent, this.contents[i + 1]);
...
ScrollSpy.prototype.isInView = function (el, nextEl) {

    var thisDims = this.getDims(el);
    if (!nextEl) {
        return (
            (thisDims.elTop < thisDims.scrollBottom)
            && (thisDims.elBottom > thisDims.scrollTop)
        )
    }
    var nextDims = this.getDims(nextEl)
    return (
        (
            (thisDims.elTop < thisDims.scrollBottom)
            && (thisDims.elBottom > thisDims.scrollTop)
        ...
        (
            thisDims.elBottom < thisDims.scrollTop &&
            nextDims.elTop > nextDims.scrollBottom
        )
    );
};

ScrollSpy.prototype.getDims = function(el) {
    var winH = this.winH,
